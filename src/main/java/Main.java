...
public class Main {
   ...
   public static void main(String[] args) throws NoSuchProviderException, NoSuchAlgorithmException, InvalidKeyException, SignatureException {
        ...
        tx3.addOutput(4, pk_scrooge.getPublic());
       ...
       tx4.signTx(pk_scrooge.getPrivate(), 0);
        ...
        System.out.println("txHandler.handleTxs(new Transaction[]{tx2}) returns: " +
            txHandler.handleTxs(new Transaction[]{tx3, tx2}).length + " transaction(s)");

       System.out.println("txHandler.handleTxs(new Transaction[]{tx2}) returns: " +
               txHandler.handleTxs(new Transaction[]{tx4}).length + " transaction(s)");
    }
